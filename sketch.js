 ...
 *
 * Also includes code from https://geraintluff.github.io/sha256/
 *
 * ToDo:
 *  - Souls
 *  - Staggering
...
function process() {
  hashvatar.reset();
  let hashedValue = sha256(input.value());
  console.log(`hashedValue: ${hashedValue}`);
  for (let i = 0; i < (sectionCount * ringCount); i++) {
    let dig = hashedValue.substring(i*2, (i*2)+2);
    let sliceValue = parseInt(Number(`0x${dig}`));
    console.log(`dig: ${dig}; sliceValue: ${sliceValue}`);
    hashvatar.addValue(sliceValue);
  }
    hashvatar.display();
}
...
// sha256 function: source: https://geraintluff.github.io/sha256/
